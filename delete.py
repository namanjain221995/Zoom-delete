"""
Zoom -> S3 historical backfill + optional Zoom cleanup.
...
AUDIT_LOG_PATH = os.getenv("AUDIT_LOG_PATH", "zoom_s3_delete_audit.jsonl").strip()
...
TEXTUAL_FILE_TYPES = {"TRANSCRIPT", "CHAT"}
...
def process_meeting(
    ...
    start_time = meeting.get("start_time", "") or ""
    topic = meeting.get("topic", "") or ""
    host_email = (host_user.get("email") or "").strip().lower()
    host_name = build_host_name(host_user)

    if ONLY_MEETING_ID:
        if normalize_meeting_id_value(meeting_id) != normalize_meeting_id_value(ONLY_MEETING_ID):
            return
    ...
    if not meeting_id or not meeting_uuid:
        print("Skipping meeting because meeting_id or meeting_uuid is missing")
        bump_stat("meetings_skipped_missing_ids", department_folder, 1)
        return

    try:
        detail = get_recording_detail(token_manager, meeting_uuid, meeting_id, requested_start_time=start_time)
    except Exception as e:
        bump_stat("failed", department_folder, 1)
        print(f"Failed to fetch recording detail: {e}")
        return

    list_files = extract_recording_files(meeting)
    detail_files = extract_recording_files(detail)
    recording_files = merge_recording_file_lists(list_files, detail_files)
    detail_tokens = merge_download_tokens(detail, meeting)

    if DETAIL_DEBUG:
        print(
            f"[FILE SOURCES] meeting_id={meeting_id}; meeting_uuid={meeting_uuid}; "
            f"list_files={len(list_files)}; detail_files={len(detail_files)}; merged_files={len(recording_files)}; "
            f"meeting_recording_count={meeting.get('recording_count')}; detail_recording_count={detail.get('recording_count')}; "
            f"meeting_total_size={meeting.get('total_size')}; detail_total_size={detail.get('total_size')}"
        )

    if not recording_files:
        bump_stat("meetings_without_recording_files", department_folder, 1)
        print(
            "No recording files found "
            f"(list_files={len(list_files)}, detail_files={len(detail_files)}, "
            f"recording_count={detail.get('recording_count')}, total_size={detail.get('total_size')}, "
            f"detail_uuid={detail.get('uuid')}, detail_start_time={detail.get('start_time')})"
        )
        return

    # ── UPDATED participants logic ──────────────────────────────────────────
    # HR + Advanced-Training REQUIRE participants (person/candidate resolution),
    # so they override SKIP_PARTICIPANTS_LOOKUP. Training needs them only when
    ...
    # Advanced-Training: full candidate list manifest
...
def process_host(
    ...
    host_email = (user.get("email") or "").strip().lower()
    user_start_date = safe_user_start_date(user)

    print("\n" + "#" * 120)
    print(f"PROCESSING HOST: {host_name} <{host_email}> id={user_id}")
    print(f"Department      : {department_folder}")
    print(f"Auto window     : {user_start_date} -> {end_date}")

    for from_date, to_date in iter_month_windows(user_start_date, end_date):
        try:
            meetings = list_user_recordings(token_manager, user_id, from_date, to_date)
        except Exception as e:
            print(f"Failed listing recordings for host={host_email or user_id} from={from_date} to={to_date}: {e}")
            continue

        for meeting in meetings:
            meeting_key = (
                str(department_folder or ""),
                str(user_id or ""),
                str(meeting.get("id") or ""),
                str(meeting.get("uuid") or ""),
                str(meeting.get("start_time") or ""),
            )

            with seen_meetings_lock:
...
def main():
    end_date = today_utc_date_str()

    print("=== ZOOM -> S3 -> OPTIONAL ZOOM FILE DELETE START (Lambda-aligned layout) ===")
    print(f"AWS_REGION                        = {AWS_REGION}")
    print(f"ZOOM_SECRET_NAME                  = {ZOOM_SECRET_NAME}")
    print(f"S3_BUCKET_NAME                    = {S3_BUCKET_NAME}")
    print(f"TARGET_DEPARTMENT_IN              = {TARGET_DEPARTMENT_INPUT}")
    print(f"TARGET_DEPARTMENTS                = {TARGET_DEPARTMENTS}")
    print(f"MIN_SCAN_DATE                     = {MIN_SCAN_DATE}")
    print(f"END_DATE                          = {end_date}")
    print(f"ONLY_HOST_EMAIL                   = {ONLY_HOST_EMAIL or '(all)'}")
    print(f"ONLY_MEETING_ID                   = {ONLY_MEETING_ID or '(all)'}")
    print(f"SKIP_PARTICIPANTS                 = {SKIP_PARTICIPANTS_LOOKUP}")
    print(f"DRY_RUN                           = {DRY_RUN}")
    print(f"DELETE_FROM_ZOOM                  = {DELETE_FROM_ZOOM}")
    print(f"DELETE_ACTION                     = {DELETE_ACTION}")
    print(f"PREFER_UUID_RECORDS               = {PREFER_UUID_RECORDS}")
    print(f"ALLOW_UNSAFE_MEETING_ID_FALLBACK  = {ALLOW_UNSAFE_MEETING_ID_FALLBACK}")
    print(f"DETAIL_DEBUG                      = {DETAIL_DEBUG}")
    print(f"REQUIRE_S3_SIZE_MATCH             = {REQUIRE_S3_SIZE_MATCH}")
    print(f"TRANSCRIPT_RECHECK_ATTEMPTS       = {TRANSCRIPT_RECHECK_ATTEMPTS}")
    print(f"TRANSCRIPT_RECHECK_INITIAL_SLEEP  = {TRANSCRIPT_RECHECK_INITIAL_SLEEP_SECONDS}")
    print(f"TRANSCRIPT_RECHECK_MAX_SLEEP      = {TRANSCRIPT_RECHECK_MAX_SLEEP_SECONDS}")
    print(f"HOST_WORKERS                      = {HOST_WORKERS}")
    ...
    print(f"SF_SECRET_NAME                    = {SF_SECRET_NAME}")
    print(f"FOLDER_FUZZY_THRESHOLD            = {FOLDER_FUZZY_THRESHOLD}")
    ...
    users = list_all_users(token_manager)
    print(f"total_users_in_zoom               = {len(users)}")
    ...
    for user in users:
        ...
        users_by_department[dept_folder].append(user)

    for dept in TARGET_DEPARTMENTS:
        set_hosts_matched(dept, len(users_by_department.get(dept, [])))

    seen_meetings = set()
    seen_meetings_lock = threading.Lock()

    for department_folder in DEPARTMENT_PRIORITY:
        if department_folder not in TARGET_DEPARTMENTS:
            continue

        dept_users = users_by_department.get(department_folder, [])
        print("\n" + "#" * 140)
        print(f"STARTING DEPARTMENT: {department_folder}")
        print(f"HOSTS IN DEPARTMENT: {len(dept_users)}")
            ...
            with ThreadPoolExecutor(max_workers=HOST_WORKERS) as host_executor:
                ...
                for future in as_completed(host_futures):
                    try:
                        ...
                        bump_stat("failed", department_folder, 1)
                        print(f"Host processing failed in department {department_folder}: {e}")

        print(f"Completed department: {department_folder}")
        print(json.dumps(snapshot_stats()["department_stats"][department_folder], indent=2))
